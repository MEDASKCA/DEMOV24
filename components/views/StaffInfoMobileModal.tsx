...
export default function StaffInfoMobileModal({
  ...
  const staffDetails = {
    employeeId: 'TS' + Math.floor(Math.random() * 9000 + 1000),
    grade: isSenior ? 'Consultant' : 'Band 7',
    shiftStart: '08:00',
    shiftEnd: '18:00',
    currentLocation: staff.theatre || 'Main Theatre 4',
    breaksTaken: Math.floor(Math.random() * 2),
    breaksScheduled: 2,
    lastBreak: Math.random() > 0.5 ? '12:15 PM (30 mins)' : null,
    status: Math.random() > 0.7 ? 'On scheduled break' : 'On duty',
    casesToday: Math.floor(Math.random() * 3) + 1,
    efficiency: 92 + Math.floor(Math.random() * 8),
    hoursWorked: 6.5 + Math.random() * 2,
    overtimeThisWeek: Math.floor(Math.random() * 5),
    ...
    competencies: !isSenior
      ...
      : null,
    reliefAvailable: !isSenior && Math.random() > 0.5,
    performance: {
      casesCompleted: Math.floor(Math.random() * 3) + 1,
      efficiency: 92 + Math.floor(Math.random() * 8),
      onTimeStart: Math.random() > 0.2
    }
  };
